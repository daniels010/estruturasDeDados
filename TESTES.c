...
void Incluir()
{
  ...
  do
  {
    ...
    if(cont > 0){
        strcpy(chave, Pessoas.cpf);
        do
        {
        fread(&Pessoas, tamALOC, 1, Crediario);
        if (strcmp(Pessoas.cpf, chave) == 0)
        {
            Achou = 1;
            printf("EXISTE\n");
            break;
        }
        } while (!feof(Crediario) && (Achou == 0));
        if(Achou == 1){
        system("pause");
        break;
        }
        if(Achou == 0){
            printf("Nome: ");
            scanf("%s", Pessoas.nome);
            strcpy(Pessoas.cpf, chave);
            fseek(Crediario, 0, 2);
            fwrite(&Pessoas, tamALOC, 1, Crediario); // HORA DE REFLEXAO!!
            cont++;
        }
    }
    printf("\nNova inclusao? S/N ");
